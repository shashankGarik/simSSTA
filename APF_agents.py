...
class APFAgents():
    def __init__(self,obstacles,controller,frame_rate,infinity):
        self.car_pos = np.array([[-50.0, 300.0, 0.0, 0.0, 0, 15, 3]])# startx,starty,vx,vy,colour,radius,shape(agent)
        # goal = np.array([[800, 1500],[700, 1600]])  # global_goal_x,global_goal,y,local_goal_x(intersection_x),local_goal_y(intersection_y)
        self.goal_pos = np.array([[800, 1500]]) # the new one is an object
        ...
        self.random_agent_generate_time=100000 #change to generate varied agents at shorter or longer time
